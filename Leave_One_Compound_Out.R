...
index <- read.csv("data/Compound_Index.csv", header=TRUE)
...
## Leave-One-Out-Compound for compound alone
set.seed(43333)
dat <- cbind(index, Compound)
subs <- unique(dat$Compound)
model_these <- vector(mode = "list", length= length(subs))
for(i in seq_along(subs))
  model_these[[i]] <- which(dat$Compound != subs[i])
names(model_these) <- paste0("Compound", subs)
LOCO_compound_alone <- train(x= dat[, 3:26],
                             y= dat[, 2],
                             method= "J48",
                             trControl=trainControl(method="cv",
                                                    index = model_these,
                                                    classProbs= TRUE))
pred <- predict(LOCO_compound_alone, dat)
actual_LOCO_compound_alone <- dat$Activity
matrix_LOCO_compound_alone <- table(pred, actual_LOCO_compound_alone)
###LOCO for Protein Alone
set.seed(54444)
...
for(i in seq_along(subs))
  model_these[[i]] <- which(dat$Compound != subs[i])
names(model_these) <- paste0("Compound", subs)
LOCO_protein_alone <- train(x= dat[, 3:128],
                            y= dat[, 2],
                            method= "J48",
                            trControl=trainControl(method="cv",
                                                   index = model_these,
                                                   classProbs= TRUE))
pred <- predict(LOCO_protein_alone, dat)
actual_LOCO_protein_alone <- dat$Activity
matrix_LOCO_protein_alone <- table(pred, actual_LOCO_protein_alone)
## LOCO for crossterms
set.seed(53423)
c <- Compound
p <- Protein
...
for(i in seq_along(subs))
  model_these[[i]] <- which(dat$Compound != subs[i])
names(model_these) <- paste0("Compound", subs)
LOCO_crossterms <- train(x= dat[, 3:3026],
                         y= dat[, 2],
                         method= "J48",
                         trControl=trainControl(method="cv",
                                                index = model_these,
                                                classProbs= TRUE))
pred <- predict(LOCO_crossterms, dat)
actual_LOCO_crossterms <- dat$Activity
matrix_LOCO_crossterms <- table(pred, actual_LOCO_crossterms)
####LOCO for protein_protein Crossterms
set.seed(90099)
...
for(i in seq_along(subs))
  model_these[[i]] <- which(dat$Compound != subs[i])
names(model_these) <- paste0("Compound", subs)
LOCO_PxP_crossterms <- train(x= dat[, 3:7875],
                             y= dat[, 2],
                             method= "J48",
                             trControl=trainControl(method="cv",
                                                    index = model_these,
                                                    classProbs= TRUE))
pred <- predict(LOCO_PxP_crossterms, dat)
actual_LOCO_PxP_crossterms <- dat$Activity
matrix_LOCO_PxP_crossterms <- table(pred, actual_LOCO_PxP_crossterms)
...
for(i in seq_along(subs))
  model_these[[i]] <- which(dat$Compound != subs[i])
names(model_these) <- paste0("Compound", subs)
LOCO_CxC_crossterms <- train(x= dat[, 3:129],
                             y= dat[, 2],
                             method= "J48",
                             trControl=trainControl(method="cv",
                                                    index = model_these,
                                                    classProbs= TRUE))
pred <- predict(LOCO_CxC_crossterms, dat)
actual_LOCO_CxC_crossterms <- dat$Activity
matrix_LOCO_CxC_crossterms <- table(pred, actual_LOCO_CxC_crossterms)
## Statistical Assessment
set.seed(45555)
z <- data.frame(matrix_LOCO_compound_alone)
y <- data.frame(matrix_LOCO_protein_alone)
x <- data.frame(matrix_LOCO_crossterms)
w <- data.frame(matrix_LOCO_PxP_crossterms)
v <- data.frame(matrix_LOCO_CxC_crossterms)
...
set.seed(433330)
pdf("Performance_of_Leave_One_Compound_Out.pdf")
